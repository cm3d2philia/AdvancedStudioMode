...
namespace CM3D2.AdvancedStudioMode.Plugin
{
    [PluginVersion("23.1.3")]
    [PluginFilter("COM3D2x64")]
    [PluginFilter("CM3D2x86")]
    [PluginFilter("CM3D2VRx64")]
    [PluginName("AdvancedStudioMode")]
    [PluginFilter("CM3D2OHx64")]
    [PluginFilter("CM3D2OHVRx64")]
    [PluginFilter("CM3D2OHx86")]
    [PluginFilter("COM3D2_Trialx64")]
    [PluginFilter("CM3D2x64")]
    public partial class AdvancedStudioMode : PluginBase
	{
		...
		{
			MaidProp maidProp = AdvancedStudioMode.GetFieldValue<Maid, MaidProp[]>(maid, "m_aryMaidProp")[(int)idx];
		...
		private void createMyRoomObject(string filename)
		{
			...
			gDogu[doguCnt].transform.position = gameObject4.transform.position;
			mDogu[doguCnt] = gDogu[doguCnt].AddComponent<MouseDrag6>();
			mDogu[doguCnt].isScale = false;
			mDogu[doguCnt].obj = gDogu[doguCnt];
			mDogu[doguCnt].maid = gameObject4;
			mDogu[doguCnt].angles = gameObject4.transform.eulerAngles;
		...
		private bool getModKeyPressing(AdvancedStudioMode.modKey key)
		{
			bool result;
			switch (key)
			{
			case AdvancedStudioMode.modKey.Shift:
				result = (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift));
				break;
			case AdvancedStudioMode.modKey.Alt:
				result = (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt));
				break;
			case AdvancedStudioMode.modKey.Ctrl:
		...
		private void FoceKuchipakuUpdate2(float f_fNowTime, Maid maid, int i)
		{
			if (!isFadeOut)
			{
				if (m_baKuchipakuPattern[i] != null)
				{
					...
					if (0 < num && num < num2)
					{
						...
						if (sceneLevel == 5)
						{
							if (maid.boMabataki && isFace[i])
							{
								...
								float[] fieldValue = AdvancedStudioMode.GetFieldValue<TMorph, float[]>(morph, "BlendValues");
								float[] fieldValue2 = AdvancedStudioMode.GetFieldValue<TMorph, float[]>(morph, "BlendValuesBackup");
		...
		internal static TResult GetFieldValue<T, TResult>(T inst, string name)
		{
			...
			{
				FieldInfo fieldInfo = AdvancedStudioMode.GetFieldInfo<T>(name);
				if (fieldInfo == null)
				{
					result = default(TResult);
				}
		...
		private static string[] ProcScriptBin(Maid maid, byte[] cd, string filename, bool f_bTemp)
		{
			...
			List<AdvancedStudioMode.LastParam> list = new List<AdvancedStudioMode.LastParam>();
			...
			try
			{
				...
				list.Sort((AdvancedStudioMode.LastParam a, AdvancedStudioMode.LastParam b) => a.nOrder - a.nOrder);
				for (int j = 0; j < list.Count; j++)
				{
					AdvancedStudioMode.LastParam lastParam = list[j];
        ...
        public AdvancedStudioMode() : base()
        {
            isF2 = false;
            isF3 = false;
            isSF1 = false;
            ...
            existPose = false;
            poseIniStr = "";
            poseCount = new int[100];
            delCount = new int[100];
            delCount2 = new int[100];
            maxMaidCnt = 100;
            maidCnt = 0;
            cameraIti = new Vector3(0f, 0.9f, 0f);
            cameraItiDistance = 3f;
            armL = new Vector3[100];
            isInit = false;
            myIndex = 0;
            kankyoIndex = 0;
            itemIndexB = 0;
            poseIndex = new int[100];
            itemIndex = new int[100];
            itemIndex2 = new int[100];
            faceIndex = new int[100];
            faceBlendIndex = new int[100];
            headEyeIndex = new int[100];
            copyIndex = 0;
            isCopy = false;
            okFlg = false;
            isDanceStart1 = false;
            ...
            danceFace = new float[100];
            isF6 = false;
            isF6S = false;
            dancePos = new Vector3[100];
            danceRot = new Quaternion[100];
            maxPage = 20;
            danceCheck = new float[10];
            danceCheckIndex = 0;
            danceWait = 0;
            danceCount = 0;
            bgmIndex = 0;
            effectIndex = 0;
            cubeSize = 0.12f;
            doguIndex = new List<int>();
            doguSelectIndex = 0;
            doguObject = new List<GameObject>();
            ...
            FaceName2 = new string[100];
            FaceName3 = new string[100];
            FaceTime = new float[100];
            keyFlg = false;
            xFlg = new bool[100];
            zFlg = new bool[100];
            cafeFlg = new bool[100];
            cafeCount = new int[100];
            isBone = new bool[100];
            isBoneN = new bool[100];
            isChange = new bool[100];
            ui_cam_hide_list_ = new List<UICamera>();
            isScreen = false;
            isScreen2 = false;
            isGui = false;
            goSlot = new List<TBodySkin>[500];
            bodyHit = new List<TBodyHit>[500];
            eyeL = new Vector3[500];
            eyeR = new Vector3[500];
            shodaiFlg = new bool[500];
            comboBoxControl = new ComboBox2();
            listStyle = new GUIStyle();
            listStyle2 = new GUIStyle();
            listStyle3 = new GUIStyle();
            listStyle4 = new GUIStyle();
            faceCombo = new ComboBox2();
            faceInitFlg = false;
            isPose = new bool[100];
            poseCombo = new ComboBox2();
            poseInitFlg = false;
            kankyoInitFlg = false;
            kankyo2InitFlg = false;
            isPoseInit = false;
            poseGroupIndex = 0;
            isPoseIti = new bool[100];
            poseIti = new Vector3[100];
            poseGroupCombo = new ComboBox2();
            itemCombo = new ComboBox2();
            bgmCombo = new ComboBox2();
            itemCombo2 = new ComboBox2();
            myCombo = new ComboBox2();
            bgCombo2 = new ComboBox2();
            bgCombo = new ComboBox2();
            slotCombo = new ComboBox2();
            doguCombo2 = new ComboBox2();
            doguCombo = new ComboBox2[9];
            doguComboList = new List<GUIContent[]>();
            parCombo = new ComboBox2();
            parCombo1 = new ComboBox2();
            lightCombo = new ComboBox2();
            kankyoCombo = new ComboBox2();
            isStop = new bool[100];
            maidArray = new Maid[100];
            danceName = new string[100];
            ...
            isDanceStop = false;
            isFace = new bool[100];
            isMabataki = new bool[100];
            mekure1 = new bool[100];
            mekure2 = new bool[100];
            zurasi = new bool[100];
            mekure1n = new bool[100];
            mekure2n = new bool[100];
            zurasin = new bool[100];
            hanten = new bool[100];
            hantenn = new bool[100];
            isHanten = false;
            kotei = new bool[100];
            voice1 = new bool[100];
            voice1n = new bool[100];
            voice2 = new bool[100];
            voice2n = new bool[100];
            pHandL = new int[100];
            pHandR = new int[100];
            isLook = new bool[100];
            lookX = new float[100];
            lookY = new float[100];
            isLookn = new bool[100];
            lookXn = new float[100];
            lookYn = new float[100];
            isPoseEdit = false;
            isFaceEdit = false;
            isNamida = false;
            isTear1 = false;
            ...
            doguList = new List<int>();
            parList = new List<int>();
            doguCnt = 0;
            selectLightIndex = 0;
            doguB2Index = 0;
            doguBIndex = new int[9];
            parIndex = 0;
            parIndex1 = 0;
            isVR = false;
            isVR2 = true;
            ...
            saveScene2 = 0;
            loadScene = 0;
            loadPose = new string[100];
            isLoadPose = new bool[100];
            loadCount = new int[100];
            kankyoLoadFlg = false;
            nameFlg = false;
            kankyoMax = 0;
            date = new string[10];
            ...
            ikBui = 0;
            ikMode = new int[100];
            ikModeOld = new int[100];
            ikMode2 = 0;
            ikModeOld2 = 0;
            editMaid = 0;
            isScript = false;
            mDogu = new MouseDrag6[999];
            gDogu = new GameObject[999];
            mBg = new MouseDrag6();
            gBg = new GameObject();
            mLight = new MouseDrag6[999];
            gLight = new GameObject[999];
            ikLeftArm = new IK();
            mHead2 = new MouseDrag5[100];
            mMaid2 = new MouseDrag5[100];
            mMaid = new MouseDrag2[100];
            gMaid = new GameObject[100];
            mMaidC = new MouseDrag2[100];
            gMaidC = new GameObject[100];
            mHead = new MouseDrag3[100];
            gHead = new GameObject[100];
            gMaid2 = new GameObject[100];
            gHead2 = new GameObject[100];
            mJotai = new MouseDrag3[100];
            gJotai = new GameObject[100];
            mKahuku = new MouseDrag3[100];
            gKahuku = new GameObject[100];
            gIKMuneL = new GameObject[100];
            mIKMuneL = new MouseDrag[100];
            gIKMuneR = new GameObject[100];
            mIKMuneR = new MouseDrag[100];
            vIKMuneL = new Vector3[100];
            vIKMuneR = new Vector3[100];
            vIKMuneLSub = new Vector3[100];
            vIKMuneRSub = new Vector3[100];
            haraCount = new int[100];
            haraPosition = new Vector3[100];
            muneIKL = new bool[100];
            muneIKR = new bool[100];
            gIKHandL = new GameObject[100];
            mIKHandL = new MouseDrag4[100];
            gIKHandR = new GameObject[100];
            mIKHandR = new MouseDrag4[100];
            gHandL = new GameObject[100];
            mHandL = new MouseDrag[100];
            gArmL = new GameObject[100];
            mArmL = new MouseDrag[100];
            gFootL = new GameObject[100];
            mFootL = new MouseDrag[100];
            gHizaL = new GameObject[100];
            mHizaL = new MouseDrag[100];
            gHandR = new GameObject[100];
            mHandR = new MouseDrag[100];
            gArmR = new GameObject[100];
            mArmR = new MouseDrag[100];
            gFootR = new GameObject[100];
            mFootR = new MouseDrag[100];
            gHizaR = new GameObject[100];
            mHizaR = new MouseDrag[100];
            gClavicleL = new GameObject[100];
            mClavicleL = new MouseDrag[100];
            gClavicleR = new GameObject[100];
            mClavicleR = new MouseDrag[100];
            gFinger = new GameObject[100, 30];
            mFinger = new MouseDrag[100, 30];
            gFinger2 = new GameObject[100, 12];
            mFinger2 = new MouseDrag[100, 12];
            gNeck = new GameObject[100];
            mNeck = new MouseDrag3[100];
            gSpine = new GameObject[100];
            mSpine = new MouseDrag3[100];
            gSpine0a = new GameObject[100];
            mSpine0a = new MouseDrag3[100];
            gSpine1 = new GameObject[100];
            mSpine1 = new MouseDrag3[100];
            gSpine1a = new GameObject[100];
            mSpine1a = new MouseDrag3[100];
            gPelvis = new GameObject[100];
            mPelvis = new MouseDrag3[100];
            HandL = null;
            UpperArmL = null;
            ForearmL = null;
            Head = null;
            ...
            Clavicle = null;
            IK_hand = null;
            IKHandL = new Transform[100];
            IKHandR = new Transform[100];
            IKMuneL = new Transform[100];
            IKMuneR = new Transform[100];
            IKMuneLSub = new Transform[100];
            IKMuneRSub = new Transform[100];
            Neck = new Transform[100];
            Pelvis2 = new Transform[100];
            Spine12 = new Transform[100];
            Spine0a2 = new Transform[100];
            Spine2 = new Transform[100];
            Spine1a2 = new Transform[100];
            Head1 = new Transform[100];
            Head2 = new Transform[100];
            Head3 = new Transform[100];
            HandL1 = new Transform[100];
            UpperArmL1 = new Transform[100];
            ForearmL1 = new Transform[100];
            HandR1 = new Transform[100];
            UpperArmR1 = new Transform[100];
            ForearmR1 = new Transform[100];
            HandL2 = new Transform[100];
            UpperArmL2 = new Transform[100];
            ForearmL2 = new Transform[100];
            HandR2 = new Transform[100];
            UpperArmR2 = new Transform[100];
            ForearmR2 = new Transform[100];
            ClavicleL1 = new Transform[100];
            ClavicleR1 = new Transform[100];
            Finger = new Transform[100, 40];
            Finger2 = new Transform[100, 18];
            isSavePose = false;
            isSavePose2 = false;
            isSavePose3 = false;
            isSavePose4 = false;
            ...
            isEdit = new bool[100];
            isEditNo = 0;
            isLoadFace = new bool[100];
            isDanceChu = false;
            isMessage = false;
            isPref = false;
            thum_byte_to_base64_ = string.Empty;
            depth_field_ = GameMain.Instance.MainCamera.GetComponent<DepthOfFieldScatter>();
            sepia_tone_ = GameMain.Instance.MainCamera.GetComponent<SepiaToneEffect>();
            SkirtListArray = new DynamicSkirtBone[500][];
            JumpChkTArray = new Transform[100];
            JumpChkPosArray = new Vector3[100];
            gt = new GameObject[70];
            wv = new Vector3[70];
            bGui = false;
            bGuiMessage = false;
            screenSize = new Vector2(0f, 0f);
            ...
            waku = null;
            waku2 = null;
            sortList = new List<AdvancedStudioMode.SortItem>();
            sortListMy = new List<AdvancedStudioMode.SortItemMy>();
            itemDataList = new List<AdvancedStudioMode.ItemData>();
            itemDataListMod = new List<AdvancedStudioMode.ItemData>();
            itemDataListNMod = new List<AdvancedStudioMode.ItemData>();
            isIdx1 = false;
            isIdx2 = false;
            isIdx3 = false;
            isIdx4 = false;
            gizmoHandL = new GizmoRender[100];
            gizmoHandR = new GizmoRender[100];
            gizmoFootL = new GizmoRender[100];
            gizmoFootR = new GizmoRender[100];
        }
		...
		private List<AdvancedStudioMode.SortItem> sortList;
		private List<AdvancedStudioMode.SortItemMy> sortListMy;
		private List<AdvancedStudioMode.ItemData> itemDataList;
		private List<AdvancedStudioMode.ItemData> itemDataListMod;
		private List<AdvancedStudioMode.ItemData> itemDataListNMod;
		...
		private class LastParam
		{
			public LastParam(int f_nOrder, string f_strComm, params string[] f_argArgs)
			{
				nOrder = f_nOrder;
				strComm = f_strComm;
				aryArgs = new string[f_argArgs.Length];
				f_argArgs.CopyTo(aryArgs, 0);
			}
			public int nOrder;
		...
		private class ItemData
		{
			...
			{
				AdvancedStudioMode.ItemData itemData = obj as AdvancedStudioMode.ItemData;
				return itemData != null && name == itemData.name && menu == itemData.menu;
			}
		...
		public class ItemData2
		{
			public ItemData2(CsvParser csv, int csv_y)
			{
				...
				seasonal = !string.IsNullOrEmpty(cellAsString);
				if (seasonal)
				{
